...
def run_demo(net, action_net, image_provider, height_size, cpu):
    ...
    orig_img = image_provider.copy()
    print(i)

    if i % 1 == 0:
        heatmaps, pafs, scale, pad = infer_fast(net, image_provider, height_size, stride, upsample_ratio, cpu)
        ...
        for pose in current_poses:
            pose.img_pose = pose.draw(image_provider, show_draw=True)
            crown_proportion = pose.bbox[2] / pose.bbox[3]  # 宽高比
            pose = action_detect(action_net, pose, crown_proportion)
        ...
        image_provider = cv2.addWeighted(orig_img, 0.6, image_provider, 0.4, 0)
...
def detect_main(video_source='', image_source='', video_name=''):
    ...
    net = jit.load('./weights/openpose.jit')

    # *************************************************************************
    action_net = jit.load('./action_detect/checkPoint/action.jit')
    # ************************************************************************
