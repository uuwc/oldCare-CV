...
#print(id_card_to_name.items())
#print(id_card_to_type.items())
            ...
            if facial_expression_label != 'neutral':  # alert
                if facial_expression_timing == 0:  # just start timing
                    ...
                    if difference < facial_expression_limit_time:
                        print('[INFO] %s, 房间, %s%s了 %.1f 秒. 忽略.' % (current_time, id_card_to_name[name], emotiondict[facial_expression_label],difference))
                    else:  # he/she is really smiling
                        event_desc = '%s正在%s' % (id_card_to_name[name], emotiondict[facial_expression_label])
                        event_location = '房间'
                        print('[EVENT] %s, 房间, %s正在%s.' % (current_time, id_card_to_name[name], emotiondict[facial_expression_label]))
